...
public class PassOneAssembler {
    ...
    public static void main(String[] args) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("input.asm"))) {
            ...
            System.out.println("Error reading input.asm file. Make sure it's in the same folder.");
            return;
        }

        poolTable.add(0); // first literal pool starts at 0
    ...
    static void processLine(String line) {
        ...
        if (line.contains("EQU")) {
            ...
            System.out.println("(AD,05) (C," + addr + ")");
            return;
        }
        ...
        if (line.contains("DS")) {
            ...
            addSymbol(symbol, LC);
            System.out.println("(DL,01) (C," + size + ")");
            LC += size;
            return;
        }
        ...
        if (line.contains("DC")) {
            ...
            addSymbol(symbol, LC);
            System.out.println("(DL,02) (C," + value + ")");
            LC++;
            return;
        }
        ...
        if (line.contains("MOVER") || line.contains("MOVEM") || line.contains("ADD") || line.contains("PRINT")) {
            ...
            if (opcode.equals("PRINT")) {
                ...
                System.out.println("(IS,04) (S," + getSymbolIndex(operand) + ")");
            } else {
                ...
                if (operand.startsWith("='")) {
                    addLiteral(operand);
                    System.out.println("(IS,02) (" + reg + ") (L," + getLiteralIndex(operand) + ")");
                } else {
                    addSymbolIfNotExists(operand);
                    System.out.println("(IS,01) (" + reg + ") (S," + getSymbolIndex(operand) + ")");
                }
            }
            LC++;
            return;
    ...
    static void printTables() {
        ...
        for (int i = 0; i < symbolTable.size(); i++) {
            ...
            System.out.println((i + 1) + ") " + s.name + "\t" + s.address);
        }

        System.out.println("\n--- LITERAL TABLE ---");
        for (int i = 0; i < literalTable.size(); i++) {
            Literal l = literalTable.get(i);
            System.out.println((i + 1) + ") " + l.value + "\t" + l.address);
        }
